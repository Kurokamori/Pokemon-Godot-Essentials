...
## [method Assets.sprite_sheet_frames] slices a spritesheet
## An icon remains on its first frame unless [method set_animating] is on
...
## Clears teh icon and stops and animations
func clear_icon() -> void:
	set_animating(false)
	set_sheet(null)
	
## Size of one frame, a caller trying to position this icon needs this information
## Returns `0` if there is no sheet
...
## Stopping returns the resting frame, so it stops on its natural rest and not in the middle of say a step
